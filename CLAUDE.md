...
- ✅ **Nav** — white logo + Montserrat-bold wordmark, solid white **"Explore membership"** button (→ `/membership`), anchor links to `#proof` / `#community` / `#summit-2027`. Same solid black bar treatment as before. (Was a ghost "Join free" button + a `#recordings` link until 2026-08-05.)
...
- **Three-step funnel (set 2026-08-06).** Nothing jumps a visitor straight to a payment form:
...
- **Nav "Explore membership"** → `/membership`. **Hero CTA "Explore membership"** → `#community` (the gold band), whose CTA "Explore membership · from $27/month" then goes to `/membership`. The Circle free-tier invitation-token join link is **no longer used on `index.html`**; it survives in git history and in the archive page.
